...
import { generateUploadToken, loadSetting } from '@/utils/qiniu';
...
export function useUpload(file: File) {
  ...
  const [observable, setObservable] = React.useState<ReturnType<
    typeof upload
  > | null>(null);
  const subscribeRef = React.useRef<ReturnType<
    ReturnType<typeof upload>['subscribe']
  > | null>(null);
  ...
  const speed = React.useMemo(() => {
    ...
    if (Array.isArray(progress.chunks)) {
      const size = progress.chunks.reduce(
        (acc, cur) => (!cur.fromCache ? cur.loaded + acc : acc),
        0,
      );
  ...
  React.useEffect(() => {
    ...
    // 线上应该使用服务端生成 token
    setToken(
      generateUploadToken({ assessKey, secretKey, bucketName, deadline }),
    );
  }, [file]);
  ...
  React.useEffect(() => {
    ...
    if (token != null) {
      ...
      setObservable(
        upload(file, file.name, token, undefined, {
          checkByMD5: true,
          debugLogLevel: 'INFO',
